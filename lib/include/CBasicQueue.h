...
//        cout<<"CBasicQueue m_capacity:"<<m_capacity<<" m_size:"<<m_size<<" len:"<<len<<" newSize:"<<newSize<<endl;
        ...
        // memory not aligned
        int alignSize = sizeof(int);
        long addr = (long)&m_array[m_size];
        int zigzag = addr % alignSize;
        if (zigzag == 0 && (len % alignSize == 0)) {
            memcpy(&m_array[m_size], bytes, sizeof(T)*len);
        } else {
            unsigned char *p = (unsigned char *)&m_array[m_size];
            for (long i = 0; i < sizeof(T)*len; i++) {
                *(p + i) = *((unsigned char*)bytes + i);
            }
        }
        m_size = newSize;
